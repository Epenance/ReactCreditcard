...
export default class App extends Component {
  render() {
    ...
    const { counter, counterActions, creditCardActions, creditcard } = this.props;
    return (
      <div>
        <h1>Simple Redux Boilerplate</h1>
        {/* notice that we then pass those unpacked props into the Counter component */}
        <Counter counter={counter} actions={counterActions} />
        <CreditCard creditcard={creditcard} actions={creditCardActions} />
        <CreditCardForm creditcard={creditcard} actions={creditCardActions} />
      </div>
    );
  }
}
...
App.propTypes = {
  ...
  creditCardActions: PropTypes.object.isRequired
};
